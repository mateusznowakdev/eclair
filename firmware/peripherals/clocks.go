...
// ConfigureCPUClock changes the GCLK0 clock frequency to 8MHz. This is a main
// clock and is used for CPU and most peripherals.
//
// This will make the USB peripheral unusable, so ConfigureUSBClock should be
// called immediately.
func ConfigureCPUClock() {
	sam.GCLK.GENDIV.Set(0 | (clockGCLK0Divider << sam.GCLK_GENDIV_DIV_Pos))
	for sam.GCLK.STATUS.HasBits(sam.GCLK_STATUS_SYNCBUSY) {
	}
}

// ConfigureUSBClock enables the GCLK5 clock at 48MHz and assigns it to the USB
// peripheral.
...
// on the current GCLK0 prescaler value.
//
// This workaround is needed because machine.CPUFrequency on SAMD21 returns a
// hardcoded value of 48MHz.
